...
import { RocketLaunch, MapTrifold, SealCheck, ChartBar, Play, Check, ArrowCounterClockwise, LockOpen, PencilSimple, Flag, Power, Copy } from '@phosphor-icons/react'
...
function DetailPanel({ state, taskId }: { state: GoDagState; taskId: string }) {
  const dt = state.dag.tasks.find(t => t.id === taskId)
  const ts = state.tasks[taskId]
  if (!dt || !ts) return null
  const statusLabel = ts.status === 'done' ? (ts.acceptance_passed !== false ? 'Passed' : 'Failed') : ts.status
...
interface Props { state: GoDagState | null; events: LogEvent[]; selected: string | null; onSelect: (id: string) => void }

export function Sidebar({ state, events, selected, onSelect }: Props) {
  ...
  return (
    <div className="w-[280px] max-xl:w-[240px] max-lg:hidden border-l border-edge-2 flex flex-col bg-surf-1">
      ...
      {selected && state && <DetailPanel state={state} taskId={selected} />}
    </div>
  )
}
