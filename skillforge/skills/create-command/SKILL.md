...
1. 创建文件: `{plugin-name}-dev/commands/{command-name}.md`
2. 从模板生成内容
3. 显示安装说明
...
📁 位置: ./{plugin-name}-dev/commands/{command-name}.md
...
4. 更新后重新安装:
   /plugin uninstall {plugin-name}-dev@local
   /plugin install {plugin-name}-dev@local
```
...
2. 询问command信息:
   - 名称: status
   - 描述: 显示当前插件状态
3. 创建 reddit-automation-dev/commands/status.md
4. 显示安装说明
